...
export class AddUserHelper {
    ...
    private getAddUserRequest(req: any): AddUserRequest {
       ...
       addUserReq.contactPhone=CommonUtil.isNotBlank(userInfo.phoneNumber)?userInfo.phoneNumber:"";
       addUserReq.contactPhoneExtn=CommonUtil.isNotBlank(userInfo.extxn)?userInfo.extxn:"";
       addUserReq.faxNumber=CommonUtil.isNotBlank(userInfo.faxNumber)?userInfo.faxNumber:"";
       addUserReq.billingAce=userInfo.ableToPayBills?"Y":"N";
       addUserReq.startStopAce=userInfo.ableToStarttopService?"Y":"N";
          ...
          addUserReq = this.setBPDtlsForRequestForBroker(req,addUserReq,req.body.customerList);
    		    ...
    		    addUserReq =this.setBPDtlsForRequestBasedonUserType(req,addUserReq,req.body.customerList);
    			  ...
    			  addUserReq = this.setBPDtlsForRequestForCustomerAdmin(req,addUserReq,req.body.customerList);
    				...
    				CommonUtil.equalsIgnoreCase(req.body.selection.subUserType, cst.USERTYPE_EXT_CUS_ASC)){
    			  addUserReq = this.setBPDtlsForRequestForBrokerAssociate(req,addUserReq,req.body.selAdminAsscList);
    			if(CommonUtil.equalsIgnoreCase(req.body.selection.subUserType, cst.USERTYPE_EXT_CUS_ASC)){
    				addUserReq.webSecurityRole = (cst.USERTYPE_SP_EXT_CUS_ASC);
    			}else{
            addUserReq = this.setBPDtlsForRequestForBrokerAssociate(req,addUserReq,req.body.customerList);
  ...
  if(CommonUtil.equalsIgnoreCase(selection.subUserType, cst.USERTYPE_INT_AGT_EMM)){
    ...
    LoggerUtil.info("END::AddUserHelper::setBPDtlsForRequestBasedonUserType()>>>>>>");
  ...
  return addUserReq;
   ...
   private setBPDtlsForRequestForCustomerAdmin(req:any,  addUserReq:AddUserRequest, propList:ManagementBP[]): AddUserRequest {
       ...
       try{
           ...
           LoggerUtil.info("ERROR======setBPDtlsForRequestForCustomerAdmin===>"+err.message)
         }
       return addUserReq
   }
   ...
   private setBPDtlsForRequestForBroker(req:any,  addUserReq:AddUserRequest, propList:ManagementBP[]): AddUserRequest {
       ...
       try{
          ...
          propList.forEach((mngemmBP:ManagementBP) => {
           ...
           LoggerUtil.info("ERROR======setBPDtlsForRequestForBroker===>"+err.message)
         }
       return addUserReq
   }
   ...
   public searchCustomersByBP(req: any) : Promise<any> {
        ...
        LoggerUtil.info(JSON.stringify(req.body))
        LoggerUtil.info(req.body.securityRole)
    ...
    private searchCustomersForAgent(req: any): Promise<any> {
        LoggerUtil.info("SEARCHING CUSTOMER FOR AGENT TYPE:::::"+req.body.securityRole)
    ...
    private searchCustomersForNonAgent(req: any): Promise<any> {
        LoggerUtil.info("SEARCHING CUSTOMER FOR NON AGENT TYPE:::::"+req.body.securityRole)
   ...
   private getRegionListForAgent(rgnList:any[], propList:any[], mngBpNumber: string, isMmngmtNodeSelected: boolean): Region[] {
    ...
    public getRegionList(rgnList:any[], propList:any[], mngBpNumber: string, isBroker: boolean, enteredBPNumber: string, treeGuid: string): Region[] {
         ...
         rgnList.forEach((rgn,i) => {
             if(CommonUtil.equalsIgnoreCase(mngBpNumber, rgn.parentBpNumber)){
                 ...
                 region.strBPAddress = CommonUtil.createBpAddressStrFromObj(rgn.bpAdress);
                 region.propertyList = this.getPropertyList(propList,rgn.bpNumber,isBroker,enteredBPNumber,region.bokerAssigned,treeGuid);
                 regionList.push(region)
             }
         })
         return regionList;
    ...
    private getPropertyList(propList:any[], regionBpNumber: string, isBroker: boolean, enteredBPNumber: string, brokerAssignedAtRegLevel: boolean,treeGUID: string): Property[] {
        ...
        var p = new Promise((resolve, reject) => {
            this.addUserService.searchBrokerAdmin(inReq).then(s => {
                if(null != s && null != s.brokerAdminList && s.brokerAdminList.length > 0){
                    LoggerUtil.info("BROKERS ADMINS FOUND FOR THE SEARCH CRITERIA::")
        ...
        var p = new Promise((resolve, reject) => {
            this.addUserService.searchCustomerAdmin(inReq).then(s => {
                if(null != s && null != s.customerAdminList && s.customerAdminList.length > 0){
                    LoggerUtil.info("CUSTOMER ADMINS FOUND FOR THE SEARCH CRITERIA::")
                    resolve(this.populateCustomerAdminList(s));
                }else{
                   LoggerUtil.info("NO CUSTOMER ADMINS FOUND FOR THE SEARCH CRITERIA::")
                   resolve({});
                }
            })
        });
    ...
    public isUsernameExistInLDAP(req: any): Promise<any> {
        ...
        LoggerUtil.info("Request====>"+JSON.stringify(inReq))
        var p = new Promise((resolve, reject) => {
    ...
    private  sendAddUserEmail(addUserRequest:AddUserRequest, req:any):boolean{
		LoggerUtil.info("START::AddUserHelper::sendAddUserEmail()>>>>>>");
  ...
  public listEMMAssociates(req:any): Promise<any>{
      return this.getAscociatedUsers(req);
  }
  ...
  private getAscociatedUsers(req: any) :Promise<any>{

     LoggerUtil.info("THE BP NUMBER:::::::::"+req.body.bpNumber);
     let userList = new Array<User>();
...
public getAscociatedUsersForCustAdmin(userName:string,req:any):Promise<any>{
   ...
   LoggerUtil.info("THE USERNAME:::::::::"+userName);
  let userList = new Array<User>();
 ...
 private  getStatusForBrokerAssignedOrNot( enteredBrokerNumber:string, brokerList:Array<string>, treeGUID:string):boolean{
	...
	private searchBrokerAdminList(searchString:string,  searchCriteria:string, req:any):Promise<any>{
		LoggerUtil.info("START::AddUserHelper::searchBrokerAdminList()>>>>>>");
    ...
    var p = new Promise((resolve, reject) => {
        this.addUserService.searchBrokerAdmin(request).then(s => {
          ...
          if(undefined !=s && null !=s.brokerAdminList && s.brokerAdminList.length >0){
            ...
            /*  let userSet= new Set(userNameList);
        			for(String username : userSet){
        				let businessName :string;
        				let user = new User();
        				user.userName = (username);
        				List<String> bpList = new ArrayList<String>();
        				for(AdminDetailsDO brokerAdmin: brokerAdminList){
        					if(username.equalsIgnoreCase(brokerAdmin.getUserName())){
        						bpName = brokerAdmin.getBpName();
        						if(StringUtils.isNotBlank(brokerAdmin.getBpAddress()))
        							bpAddress = brokerAdmin.getBpAddress();
        						user.setFirstName(brokerAdmin.getFirstName());
        						user.setLastName(brokerAdmin.getLastName());
        						user.setBillingAce(brokerAdmin.getBillingAce());
        						user.setStartStopAce(brokerAdmin.getStartStopAce());
        						bpList.add(brokerAdmin.getBpNumber());
        						businessName = brokerAdmin.getBpName();
        					}
        				}
        				user.setBusinessName(businessName);
        				user.setBpList(bpList);
        				userList.add(user);
        			}*/
            }
         });
